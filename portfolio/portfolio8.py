...
def calculate_portfolio(df):
    ...
    df.loc[mask_stock, "fee"] = (
        df.loc[mask_stock, "buy_price"] * df.loc[mask_stock, "shares"] * FEE_RATE
    )
    ...
    df["market_value"] = 0
    df["cost_basis"] = 0
    df.loc[mask_stock, "market_value"] = (
        df.loc[mask_stock, "shares"] * df.loc[mask_stock, "prev_close"]
    )
    df.loc[mask_stock, "cost_basis"] = (
        df.loc[mask_stock, "shares"] * df.loc[mask_stock, "buy_price"]
        + df.loc[mask_stock, "fee"]
    )
    ...
    df["pnl_contrib"] = (
        df["pnl_jpy"] / total_pnl * 100 if total_pnl != 0 else 0
    )
        ...
        for idx, trade in df_trades.iterrows():
            ...
            if trade_date in history.index:
                ...
                if trade["currency"] == "USD":
                    history.loc[trade_date, "Total"] += (
                        trade["shares"] * FX_TO_JPY["USD"] * mult
                    )
                else:
...
if not df_portfolio.empty:
    ...
    sector_df = (
        df_portfolio.groupby("sector")
        .agg(mv_jpy=("mv_jpy", "sum"), pnl_jpy=("pnl_jpy", "sum"))
        .reset_index()
    )
